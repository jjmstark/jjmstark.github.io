...
In the [Year in Ethereum 2021](/my-blog/posts/yie2021/), we published a chart comparing the total transaction volume of the Ethereum and Bitcoin blockchains. Last year, Ethereum transferred approximately $11.6 trillion (ETH and ERC-20s), and Bitcoin transferred approximately $4.6 trillion (BTC and USDT).
...
* For all assets, we are using the “Adjusted Transfer Value” metric from Coin Metrics. [Coin Metrics defines this](https://docs.coinmetrics.io/asset-metrics/transactions/txtfrvaladjusd) as “The USD value of the sum of native units transferred that interval removing noise and certain artifacts”.
...
The calculation and data sources for this are documented in a google sheet [here](https://docs.google.com/spreadsheets/d/1-Is51Do_AgatnUsoxo-Iy7B0clCyxf1VN_gDt8mtly4/edit#gid=1736784120), which was linked in the [original YiE 2021](https://stark.mirror.xyz/q3OnsK7mvfGtTQ72nfoxLyEV5lfYOqUfJIoKBx7BG1I) post.
...
Anytime you transfer Bitcoin, you are “using up” one of those UTXOs. So imagine you want to send only 0.5 BTC to someone. Your wallet will “spend” the whole 2 BTC UTXO, sending 0.5 to your recipient (creating a new UTXO of 0.5 BTC), and sending 1.5 BTC back to you in a new UTXO containing 1.5 BTC. Here’s a [more in-depth explanation](https://bitcoinbriefly.com/utxo-consolidation-explained/#what-is-a-utxo).
...
Around the same time, we heard back from Nishita at ARK Invest. She shared that the $13T figure from the graph came from Glassnode, specifically their “change-adjusted volume” metric. This metric is [defined by Glassnode](https://docs.glassnode.com/api/transactions#change-adjusted-volume-total) as “The total amount of coins transferred on-chain, adjusted by change volume. Only successful transfers are counted”.
